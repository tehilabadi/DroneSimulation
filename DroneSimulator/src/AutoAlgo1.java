    ...
    public void ai(int deltaTime) {
        ...
        if (!is_risky) {
            ...
            if (!try_to_escape) {
                ...
                double front = lidar0.current_distance;
                Lidar lidar1 = drone.lidars.get(1);
                double right = lidar1.current_distance;
                Lidar lidar2 = drone.lidars.get(2);
                double left = lidar2.current_distance;
                int spin_by = 0;
                int flag = 0;
                int dis = 290;

                if (!SimulationWindow.return_home) {
                    if (left > dis && left > 0.1 && delay_left == 0) {
                        spin_by = -90;
                        delay_left += 20;
                        flag = 1;
                    }
                    if (left < 90 && left > 0.1 && delay_right == 0) {
                        spin_by = 5;
                        delay_right += 5;
                        flag = 3;
                    }
                    if (right < 80 && right > 0.1 && delay_left == 0) {
                        spin_by = -8;
                        delay_left += 5;
                        flag = 4;
                    }
                    if (left > 120 && left > 0.1 && delay_left == 0) {
                        spin_by = -6;
                        delay_left += 4;
                        flag = 2;
                    }
                    if (front < 120 && front > 0.0 && delay_front == 0) {
                        spin_by = 90;
                        delay_front += 4;
                        flag = 5;
                    }
                    ...
                    if (right > dis && right > 0.1 && delay_right == 0) {
                        spin_by = 90;
                        delay_right += 20;
                        flag = 1;
                    }
                    if (right < 90 && right > 0.1 && delay_left == 0) {
                        spin_by = -5;
                        delay_left += 5;
                        flag = 3;
                    }
                    if (left < 80 && left > 0.1 && delay_right == 0) {
                        spin_by = 8;
                        delay_right += 5;
                        flag = 4;
                    }
                    if (right > 120 && right > 0.1 && delay_right == 0) {
                        spin_by = 6;
                        delay_right += 4;
                        flag = 2;
                    }
...
//					}
                ...
                System.out.println("front: " + front + " right: " + right + " left: " + left + " spin_by: " + spin_by + " flag: " + flag);
                System.out.println("delay_front: " + delay_front + " delay_right: " + delay_right + " delay_left: " + delay_left);
